...
public class TopicSubscribeService {
    ...
    public static final String topicTablePath = "resources/topic_table.json";
     ...
     public void subscribeTopic(TopicDTO dto) throws IOException{
        // JSON 파일 준비
        ...
        synchronized (topicTableLock) {
            ...
            // 데이터 준비
            List<TopicDTO> registerList = new ArrayList<>();
            ObjectMapper om = new ObjectMapper();

            // 데이터 추가
            try {
                registerList = om.readValue(file, new TypeReference<List<TopicDTO>>() {});
            } catch (MismatchedInputException ignore) {}
            registerList.add(dto);

            // 중복 제거
            List<TopicDTO> uniqueList = new ArrayList<>(new HashSet<>(registerList));
